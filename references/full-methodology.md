...
绝对金额（营收、净利润、利润、FCF、OCF、capex、现金、债务、市值、EV 等）统一用"原币种 + 亿"表示，**不跨币种换算**：美元 `$1,300亿`（非 `$130B`）、人民币 `9,300亿元`、韩元 `KRW 92.1万亿`。每股金额（EPS、股价、股息、目标价）保持原值不转"亿"。股数用"亿股"（`25.66亿股`）。倍数、比率、百分比不变。韩元报告因面值过大，绝对金额不强制"亿"。当报告中同时出现"亿"和西式缩写（M/B/T）用于绝对金额时，`report_lint.py` 判 FAIL。
