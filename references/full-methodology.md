...
3. **关键数字交叉验证**
   ...
   - 读取 `references/data-validation.md`，并用 `scripts/financial_rigor.py` 记录市值、估值和交叉验证的计算检查：<=1% 一致；>1%-5% 必须解释；>5% 必须停用该数值并回到 Tier 1。`scripts/report_audit.py` 只生成确定性人工抽检清单，绝不自动抓取数据。
4. **完整性降级**
   - 最近年报缺失:不得给 Buy。
...
绝对金额（营收、净利润、利润、FCF、OCF、capex、现金、债务、市值、EV 等）统一用"原币种 + 亿"表示，**不跨币种换算**：美元 `$1,300亿`（非 `$130B`）、人民币 `9,300亿元`、韩元 `KRW 92.1万亿`。每股金额（EPS、股价、股息、目标价）保持原值不转"亿"。股数用"亿股"（`25.66亿股`）。用户/参与度指标同样用"亿"（`36亿 DAP`、`25亿 MAU`）。倍数、比率、百分比不变。韩元报告因面值过大，绝对金额不强制"亿"。当报告中任何绝对金额、股数或用户指标使用西式缩写（M/B/T）时，`report_lint.py` 判 FAIL；每股金额、倍数、韩元报告豁免。
...
紧接首页结论，只给一条 Researchability Record；完整 A/B/C、AI 研究置信度、投资确定性和首页决策置信度规则见 `references/researchability.md`。最终判决只确认二者的区别，不重复记录。
...
- 五维打分（品牌/网络效应/转换成本/成本优势/专利/监管壁垒，任选 5 维）各 1-10 分，**每项给证据**。**lint 强制**：第 3 模块必须含一张 5+ 行评分表（列名含 `score` 或 `分数`），每行证据非空，否则 `report_lint.py` 直接判 FAIL。
- 至少选 2-3 个直接竞品，横向比增速/利润率/市占率/估值/资产负债表/产品力。**lint 强制**：报告必须含一张竞品对比表（≥2 个竞品 × ≥2 个指标），或在报告中明确声明 `无直接可比竞品` 并给出理由。
- 网络效应护城河（社交平台、双边市场、用户飞轮）必须补多期 DAU/MAU/DAP 等用户/参与度指标及同比趋势，并单独成表，不得用定性描述替代量化证据。
...
对存储、能源、航运、银行、券商、汽车、硬件供应链等利润波动大的公司，必须输出（**lint 强制**：当报告出现 ≥$50B Capex 或周期行业关键词时，第 4 模块必须含一张 3+ 行多场景估值表，含 `峰值/中枢/平准/normalized/EV-FCF` 等场景关键词，否则 `report_lint.py` 判 FAIL；非周期低 Capex 公司豁免）：
...
最终判决必须写明采用哪个基数。如果只用峰值利润支撑 Buy，评级自动降为 Watchlist 或 Avoid，除非已经证明周期中枢永久抬升且 FCF 可持续；追高风险单独说明，不得写成第五档评级。
...
流动性不再作为固定模块。只有当标的是小盘、冷门、成交异常、停牌风险、或仓位金额可能影响退出时，才在本模块加入流动性风险；大盘高流动性标的默认不写。
...
1. 第 4 模块估值是否通过。
2. 第 5 模块最大风险一旦触发，组合可承受的最大损失。
3. 是否存在真实流动性约束；若无，不单独展开。
...
#### 目标 PE 与价格线
+**lint 强制**：第 8 模块必须含目标 PE 或目标价的量化输出（关键词 + 数值），否则 `report_lint.py` 判 FAIL；纯定性描述不满足。
...
#### ⭐ 价格区间摘要（强制）
+**lint 强制**：第 8 模块必须含价格区间表，覆盖"安全边际区/观察区/高估区"中至少两档，否则 `report_lint.py` 判 FAIL；纯观察标的可声明"无价格区间"跳过，但持仓标的不得使用此例外。

必须输出**潜在安全边际区 / 观察区 / 高估区**三个价格档位，但这里只解释估值语境，不得定义交易：
...
| 2026-06-08 | sh601899 紫金矿业 | 与神华报告对比发现 5 处缺失：①报告元数据 ②Input 默认值声明 ③净债务/EV 数据 ④FCF/EV 估值口径 ⑤分档价格线 → 全部补入 Skill |
