...
description: |
  华尔街式股票脱水质检 + 10 年回本数学审判（9 模块完整版）。当用户给出一个股票代码（A 股 / 港股 / 美股），并要求做深度投资分析、估值审判、买卖判断、华尔街视角研究、对冲基金视角分析时使用。
...
- 只有在流动性结论为“构成约束”且缺少退出测算数据时，才不得给超过 5% 组合仓位建议。
...
- 如果网络效应是核心护城河，必须给当前用户规模、同比/环比或明确期间变化，以及至少一个参与度或商业化指标（如 DAU/MAU、使用时长、广告展示、价格、ARPU 或转化率）。
...
流动性不再作为固定模块，但模块 5 必须明确写出 `流动性结论：不构成约束` 或 `流动性结论：构成约束`。若构成约束，必须给出 90 日平均成交额、仓位金额、压力参与率和压力退出天数。统一公式为：

```text
压力退出天数 = 仓位金额 / (90 日平均成交额 × 压力参与率)
```

默认压力参与率为 5%；压力退出天数大于 5 个交易日时，判定为流动性约束。仓位金额缺失时只能标记“需人工复核”，不得给出超过 5% 的仓位建议。
...
- ❌ 第 9 模块不给分档价格线（只给单一目标价不够操作）
- ❌ 第 9 模块给"建议关注"等模棱两可结论（只能 Buy/Hold/Watchlist/Avoid 四选一）
...
- 报告必须包含 First-Page Verdict + Evidence Ledger + 9 个固定模块
...
- 第 8 模块必须输出仓位、加减仓条件、Pre-Mortem、Action Triggers
