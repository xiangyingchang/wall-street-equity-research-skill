...
### 2. YAML Frontmatter（强制）

报告**必须**以 YAML frontmatter 开头，包含以下字段：

```yaml
---
ticker: [TICKER] / [A股代码]
company: [公司中文名] / [Company English Name]
market: 港股 / A股 / 美股
price_h: [港股价格，如适用]
price_a: [A股价格，如适用]
currency_h: HKD / CNY / USD
currency_a: CNY / USD / HKD
pe_h: [港股 PE]
forward_pe_h: [港股 Forward PE]
pb_h: [港股 PB]
dividend_yield_h: [港股股息率]
verdict: Buy / Hold-Index / Watchlist / Avoid
created: YYYY-MM-DD
data_date: YYYY-MM-DD
source: Codex / wall-street-equity-research
---
```

**原因**：Frontmatter 让 Obsidian 可以用 Dataview 查询所有报告，也方便快速识别报告的关键结论。
3. 如需 HTML 版，参考已有的 `比亚迪-华尔街式分析报告-2026-05-30.html` 暗主题样式
...
- ❌ 输出报告不加 YAML frontmatter（影响 Obsidian 索引和 Dataview 查询）
...
- 第 11 模块遗漏置信度与复核清单
- 输出报告不加 YAML frontmatter

## Verification

- 报告必须包含 YAML frontmatter（ticker / company / market / price / pe / verdict / created / data_date / source）
